...
WORKLOADS = (
    "crud", "bugfix_failing_test", "cross_cutting_refactor",
    "warm_single_file_change", "retry_after_test_failure",
)
ENGINES = {"S0_BASELINE": ("off",), "S1_RUNTIME": ("off",),
           "S2_RUNTIME_LOOP": ("off",), "S3_FULL_STACK": ("rust", "python", "off")}
REASON_CODES = {
    "component_unavailable", "capability_handshake_failed", "environment_unavailable",
    "metric_not_exposed", "provider_telemetry_unavailable", "run_failed",
    "source_restore_failed", "toolchain_unavailable", "unsupported_platform",
}
METRICS = (
    "total_ms", "orientation_ms", "context_ms", "planning_ms", "mutation_ms",
    "tests_ms", "review_ms", "refresh_ms", "ipc_ms", "ttft_ms",
    "llm_latency_ms", "tokens_input", "tokens_output", "tokens_cache",
    "tool_schema_tokens", "cpu_ms", "peak_rss_kib", "page_faults", "io_bytes",
    "snapshot_bytes", "index_bytes", "cache_hits", "cache_misses",
    "files_reprocessed", "files_reused", "retries", "observed_cost",
)
...
def preregister(*, seed: int, repetitions: int = 10, slots=(1, 20, 100)) -> dict:
    ...
    cells = [
        {"scenario": scenario, "engine": engine, "workload": workload, "slots": slot,
         "repetition": repetition}
        for scenario in SCENARIOS for engine in ENGINES[scenario]
        for workload in WORKLOADS for slot in slots for repetition in range(1, repetitions + 1)
    ]
    random.Random(seed).shuffle(cells)
    plan = {"schema": SCHEMA, "kind": "preregistration", "seed": seed,
            "repetitions": repetitions, "slots": list(slots), "runs": cells}
...
def blocked_run(run: dict, reason: str, detail: str | None = None) -> dict:
    receipt = dict(run)
    receipt.update({"schema": SCHEMA, "kind": "run", "status": "blocked",
...
def validate_run(run: dict) -> None:
    required = ("schema", "kind", "status", "scenario", "engine", "workload", "slots",
                "repetition", "corpus_sha256", "source_commit", "environment_sha256",
                "component_versions", "metrics")
    ...
    if run["scenario"] not in SCENARIOS or run["engine"] not in ENGINES[run["scenario"]]:
    ...
    for name in METRICS:
        ...
        if not isinstance(metric, dict) or "value" not in metric or "reason" not in metric:
        ...
        elif metric["reason"] is not None or not isinstance(metric["value"], (int, float)):
            raise ProtocolError(f"metric_value_invalid:{name}")
    if run["status"] == "complete" and any(m["value"] is None for m in run["metrics"].values()):
...
def validate_dataset(document: dict) -> dict:
    ...
    for run in runs:
        validate_run(run)
    identities = {(r["corpus_sha256"], r["source_commit"], r["environment_sha256"])
                  for r in runs}
    if len(identities) > 1:
        raise ProtocolError("frozen_identity_drift")
    counts = Counter(cell_id(r["scenario"], r["engine"], r["workload"], r["slots"])
                     for r in runs if r["status"] in {"complete", "partial"})
    underfilled = sorted(cell for cell, count in counts.items() if count < 10)
    return {"valid": True, "runs": len(runs), "underfilled_cells": underfilled,
