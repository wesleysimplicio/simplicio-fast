...
| --- | --- | --- | --- |
| #236 | PARTIAL | Full suite with coverage: 653 tests + 67 subtests passed on Windows after explicit stdin/file-backed subprocess hardening in `cd0867f`; total branch coverage 85.5888% (86% displayed); release-integrity pass | Final AC matrix, >=90% changed-code coverage, Linux/Windows assets, 10 worktrees/20 readers, installed S3/S4/S5 delivery |
...
| #239 | PARTIAL | `issue239-windows-20260802-30.json`; fresh 30-run indexed receipts at 10k/100k/1M; all query gates pass; 1M p95 0.1095 ms, 2 candidates, 1 decoded record, 0 KiB additional RSS | Cross-platform receipts, physical worktrees, installed package and final coverage |
| #240 | PARTIAL | `issue240-windows-20260802-30.json`; 46 focused tests + 10 subtests pass; 30-run synthetic semantic delivery selects 1 file vs 8 legacy files and reduces median tokens 32 to 4 (87.5%) | Provider tokenizer matrix, real-task recall/token matrix, warm latency and coverage |
| #241 | PARTIAL | `7be3c94`; `issue241-windows-20260802-30.json` with 30-run binary/JSON/journal/Dev CLI receipt; installed console materialization and lifecycle tests pass; full suite now passes 653 tests + 67 subtests with Windows subprocess stdin hardening | Linux/Windows byte parity, all operations, recovery/adversarial matrix and final coverage |
...
| #244 | PARTIAL | `3e89acf`, `b1b4f89`; current parser/Mapper adapter suite passes within the 36-test + 16-subtest run; parser contract and fuzz corpus remain present | Frozen real-corpus parity, installed cross-platform matrix and final coverage |
