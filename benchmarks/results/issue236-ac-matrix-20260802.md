...
owned by an external capability or service. Current master is `?`;
...
| #339 | PARTIAL | typed `simplicio.fast.projection/v1` envelope plus Code/Knowledge/Operations producers, scoped incremental store, atomic persistence, installed smoke, cross-domain benchmark, and Rust ASCII digest/validator parity (`cargo test`: 17 passed; Clippy clean) | Rust Unicode/multiplatform parity and canonical-baseline comparison |
