...
def validate_envelope(value: Mapping[str, Any]) -> dict[str, Any]:
    ...
    required = (
        "run_id", "slot_id", "issue_id", "commit", "generation", "fence",
        "idempotency_key", "budget", *REQUIRED_HASHES,
    )
    ...
    expected = digest({key: result[key] for key in (
...
class Snapshot:
    ...
    def page(self, relative_path: str, *, offset: int = 0, limit: int = 4096) -> dict[str, Any]:
        ...
        with path.open("rb") as handle:
            if path.stat().st_size:
                ...
                data = b""
        return {
            "schema": "simplicio.fast-page/v1", "snapshot_id": self.snapshot_id,
            "path": relative_path, "offset": offset, "bytes": data,
...
class SlotExecutor:
    ...
    def open_snapshot(self, run_id: str, source_hash: str, files: Mapping[str, bytes]) -> Snapshot:
        ...
        snapshot_id = digest({"run_id": run_id, "source_hash": source_hash, "manifest": manifest})
        ...
        with self._lock:
            ...
            for name, content in files.items():
                ...
                if target.exists() and hashlib.sha256(target.read_bytes()).hexdigest() != object_hash:
                    raise FastExecutorError("snapshot_corrupt", name)
                if not target.exists():
                    target.write_bytes(content)
            metadata = {
                "schema": SNAPSHOT_SCHEMA, "snapshot_id": snapshot_id,
    ...
    def execute(
        self, envelope: Mapping[str, Any], snapshot: Snapshot,
        *, writes: Mapping[str, bytes] | None = None, runtime_available: bool = True,
        ...
        receipt = {
            ...
            "run_id": env["run_id"], "slot_id": env["slot_id"],
            "issue_id": env["issue_id"], "commit": env["commit"],
            "generation": env["generation"], "fence": env["fence"],
            "idempotency_key": key, "snapshot_id": snapshot.snapshot_id,
            "overlay_id": digest({"slot_id": env["slot_id"], "generation": env["generation"], "fence": env["fence"]}),
            "cache_hit": False, "duplicate_dispatch": False,
    ...
    @staticmethod
    def verify_receipt(receipt: Mapping[str, Any], envelope: Mapping[str, Any]) -> tuple[bool, str]:
        try:
            env = validate_envelope(envelope)
            ...
            for key in ("run_id", "slot_id", "issue_id", "commit", "generation", "fence", "idempotency_key"):
                if receipt.get(key) != env.get(key):
                    return False, f"{key}_mismatch"
            if receipt.get("status") != "VERIFIED" or receipt.get("completion_authority") != "LOOP_ONLY":
...
def make_envelope(slot: int, *, run_id: str = "run", generation: int = 1, fence: str = "f1") -> dict[str, Any]:
    value = {
        "schema": ENVELOPE_SCHEMA, "run_id": run_id, "slot_id": f"slot-{slot}",
        "issue_id": f"issue-{slot}", "commit": "a" * 40, "generation": generation,
        "fence": fence, "budget": {"max_operations": 100},
        "source_hash": "source", "tool_hash": "tool", "config_hash": "config",
        "contract_hash": "contract",
    }
    value["idempotency_key"] = digest({key: value[key] for key in (
        "run_id", "slot_id", "issue_id", "commit", "generation", "fence", *REQUIRED_HASHES,
    )})
    return value
