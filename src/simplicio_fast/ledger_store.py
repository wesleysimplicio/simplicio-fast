...
class LedgerStore:
    ...
    def _payload(event: LedgerEvent) -> bytes:
        payload = json.dumps(event.record(), ensure_ascii=False, sort_keys=True, separators=(",", ":")).encode("utf-8")
        if len(payload) > MAX_RECORD_BYTES:
            raise LedgerStoreError("record_too_large", "ledger record exceeds bounded store size")
        return payload
        ...
        with self._writer_lock():
            with self.hbp_path.open("a+b") as hbp, self.hbi_path.open("a+b") as hbi:
                ...
                if event.sequence != expected_sequence:
                    raise LedgerStoreError("sequence_mismatch", "event sequence does not follow HBI")
                ...
                hbp.flush()
                os.fsync(hbp.fileno())
                hbi.write(_INDEX_ENTRY.pack(event.sequence, offset, len(payload), digest))
                hbi.flush()
                os.fsync(hbi.fileno())
        return {
            "schema": SCHEMA,
    ...
    def read_record(self, sequence: int) -> dict[str, Any]:
        ...
        if indexed_sequence != sequence or length > MAX_RECORD_BYTES:
            raise LedgerStoreError("index_mismatch", "HBI sequence or length is invalid")
        try:
        ...
        if len(data) < header_size or data[:header_size] != _FILE_HEADER.pack(HBP_MAGIC, 1):
        ...
        if record_sequence != sequence or record_length != length or hashlib.sha256(payload).digest() != digest:
            raise LedgerStoreError("payload_digest_mismatch", "HBP payload does not match HBI")
        try:
            record = json.loads(payload.decode("utf-8"))
        except (UnicodeDecodeError, json.JSONDecodeError) as exc:
            raise LedgerStoreError("invalid_record", "HBP payload is not valid JSON") from exc
    ...
    def _event_from_record(record: dict[str, Any]) -> LedgerEvent:
        try:
            ...
            raise LedgerStoreError("invalid_record", "HBP event fields are invalid") from exc

    def verify(self) -> dict[str, Any]:
        try:
            hbi = self.hbi_path.read_bytes()
        except FileNotFoundError:
            return {"schema": SCHEMA, "status": "invalid", "reason_code": "missing_index", "events": 0}
        header_size = _FILE_HEADER.size
        if len(hbi) < header_size or hbi[:header_size] != _FILE_HEADER.pack(HBI_MAGIC, 1):
            return {"schema": SCHEMA, "status": "invalid", "reason_code": "invalid_header", "events": 0}
