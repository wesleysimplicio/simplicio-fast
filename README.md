...
Today, version 1.0.0 provides `build`, `query`, direct-index `search`, bounded `context`, typed `impact`, `stats` and `doctor`. Mapper remains the canonical public context producer; consumers should use its versioned handles rather than reading this binary directly. Full cross-repository integration is tracked in the [integration epic](https://github.com/wesleysimplicio/simplicio-fast/issues/1).
