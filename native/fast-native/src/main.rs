...
fn execute(operation: &str, payload: &Value) -> Result<Value, String> {
    match operation {
        "sha256" => {
            let bytes = hex::decode(payload["hex"].as_str().ok_or("hex")?)
                .map_err(|_| "hex")?;
        ...
        "page" => {
            ...
            if limit == 0 || limit > 65536 { return Err("bounds".into()); }
            let end = offset.saturating_add(limit).min(bytes.len());
            let page = if offset >= bytes.len() { &[] } else { &bytes[offset..end] };
        ...
        "overlay_merge" => {
            ...
            for (key, value) in overlay {
                if value.is_null() { merged.remove(&key); } else { merged.insert(key, value); }
            }
            Ok(json!(merged))
        }
...
fn main() {
    ...
    let request: Value = match serde_json::from_str(&input) {
        Ok(value) => value, Err(_) => std::process::exit(2),
    };
    if request["abi"] != ABI { std::process::exit(3); }
    match execute(request["operation"].as_str().unwrap_or(""), &request["payload"]) {
