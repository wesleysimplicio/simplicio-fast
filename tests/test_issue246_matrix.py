...
def test_matrix_preserves_raw_receipts_and_marks_partial(
    ...
    def fake_run(
        *, files: int, functions: int, repetitions: int, rust_executable: Path | None
    ) -> dict[str, object]:
        calls.append((files, functions, repetitions))
    ...
    assert calls == [(*issue246_matrix._shape(size), 10) for size in (10_000, 100_000)]
