...
class NavigationContractTest(unittest.TestCase):
    ...
    def test_definition_callers_and_callees_use_snapshot_ids(self) -> None:
        ...
        definition = self.index.navigate(target.symbol_id, "definition", "outgoing", 10, generation=self.index.generation)
        self.assertEqual("simplicio.fast-navigation/v1", definition.schema)
        self.assertEqual("simplicio.fast.provenance/v1", definition.provenance["schema"])
        ...
        callers = navigate(self.snapshot, target.symbol_id, "callers", "incoming", {"max_nodes": 10})
        self.assertIn(caller.symbol_id, callers.ids)
        callees = self.index.navigate(caller.symbol_id, "callees", "outgoing", {"max_nodes": 10})
    ...
    def test_cursor_is_opaque_bounded_and_deterministic(self) -> None:
        ...
        next_page = self.index.navigate(target.symbol_id, "callers", "incoming", 1, cursor=page.cursor)
    ...
    def test_stale_and_invalid_inputs_have_stable_reason_codes(self) -> None:
        ...
        with self.assertRaises(NavigationError) as stale:
            self.index.navigate(target.symbol_id, "definition", "outgoing", 1, generation="SFAST001:" + "0" * 64)
    ...
    def test_unmaterialized_relations_are_explicit_and_navigation_does_not_write(self) -> None:
        ...
        page = self.index.navigate(target.symbol_id, "imports", "outgoing", {"max_nodes": 1})
