...
def complete_run(repetition=1):
    return {
        "schema": "simplicio.fast.e2e-benchmark/v1", "kind": "run",
        "status": "complete", "scenario": "S3_FULL_STACK", "engine": "rust",
        "workload": "crud", "slots": 1, "repetition": repetition,
        "corpus_sha256": HASH, "source_commit": "deadbeef",
...
class E2EProtocolTest(unittest.TestCase):
    ...
    def test_blocked_run_preserves_blocked_state(self):
        ...
        run.update({"corpus_sha256": HASH, "source_commit": "deadbeef",
                    "environment_sha256": "b" * 64,
                    "component_versions": {"fast": {"version": "2.0.10"}}})
        validate_run(run)
        self.assertEqual("blocked", run["status"])
        self.assertTrue(all(metric["value"] is None for metric in run["metrics"].values()))
    ...
    def test_dataset_rejects_identity_drift(self):
        ...
        with self.assertRaisesRegex(ProtocolError, "frozen_identity_drift"):
            validate_dataset({"schema": runs[0]["schema"], "kind": "dataset", "runs": runs})

    def test_dataset_reports_underfilled_cells_without_inventing_runs(self):
        runs = [complete_run(i) for i in range(1, 10)]
        result = validate_dataset({"schema": runs[0]["schema"], "kind": "dataset", "runs": runs})
