...
from simplicio_fast.vector_contracts import VECTOR_QUERY_RECEIPT_SCHEMA, validate_vector_query_receipt
...
class TurboQuantIndexTest(unittest.TestCase):
    def test_build_query_and_contract_validated_integral_rerank(self) -> None:
        index = TurboQuantIndex.build([('near', (1.0, 0.0)), ('near-two', (0.9, 0.0)), ('orthogonal', (0.0, 1.0))], seed=7, generation='g1')
        ...
        self.assertEqual(['near', 'near-two'], [row['canonical_id'] for row in receipt['results']])
    ...
    def test_build_rejects_empty_duplicate_mixed_dimension_and_invalid_limits(self) -> None:
