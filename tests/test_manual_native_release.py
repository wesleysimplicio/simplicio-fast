...
def test_manual_runner_declares_workflow_targets():
    assert set(TARGETS) == {"linux-x86_64", "linux-aarch64", "macos-aarch64", "windows-x86_64"}
...
def test_deterministic_archive_and_verified_platforms(tmp_path):
    ...
    (native / "manifest.json").write_text(json.dumps({
        "abi": "simplicio.fast-native/v1", "platform": "linux-x86_64",
        "filename": binary.name, "version": "2.0.20", "source_commit": "a" * 40,
        "toolchain": "fixture", "size": 6, "sha256": digest,
    }), encoding="utf-8")
    core_binary = core / "simplicio-fast-rs"
    core_binary.write_bytes(b"core")
    (core / "engine-manifest.json").write_text(json.dumps({
        "schema": "simplicio.fast.engine-manifest/v1", "engine": "rust",
        "status": "available", "version": "2.0.20", "capabilities": ["stats", "query", "context"],
    ...
    assert receipt["archives"][0]["sha256"] == hashlib.sha256((output / "simplicio-fast-engines-linux-x86_64.tar.gz").read_bytes()).hexdigest()
