...
class E2EMatrixTest(unittest.TestCase):
    def test_python_matrix_freezes_identity_and_observes_s0(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            result = run_matrix(files=2, functions=3, repetitions=10, repo_root=Path(directory))
        ...
        self.assertEqual("whitespace-v1-estimate", result["scenarios"]["S0_BASELINE"]["token_measurement"])
    ...
    def test_unavailable_cells_are_null_with_reason_codes(self) -> None:
        ...
        for name in ("S1_RUNTIME", "S2_RUNTIME_LOOP", "S3_FULL_STACK"):
            ...
            self.assertTrue(all(value is None for value in scenario["metrics"].values()))
